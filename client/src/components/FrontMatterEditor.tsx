...
import { Plus, X, ChevronDown, ChevronRight, Tag, Calendar, Layout, Image, Globe, Hash } from "lucide-react";
...
const STANDARD_FIELDS = [
  ...
  { key: "layout", label: "Layout", icon: Layout, type: "select", options: ["post", "page", "default", "home"] },
...
export default function FrontMatterEditor({ frontMatter, onChange, siteId }: Props) {
  const [showCustom, setShowCustom] = useState(false);
  const [newKey, setNewKey] = useState("");
  const [newVal, setNewVal] = useState("");
  const [expandedSections, setExpandedSections] = useState({ standard: true, seo: false, custom: false });

  const update = (key: string, value: unknown) => onChange({ ...frontMatter, [key]: value });
  ...
  const customKeys = Object.keys(frontMatter).filter(k =>
    !STANDARD_FIELDS.map(f => f.key).includes(k) && k !== "tags" && k !== "categories"
  );
  ...
  return (
    <div className="text-sm">
      <div className="px-3 py-2.5 border-b border-border">
        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Front Matter</span>
      </div>
      ...
      {expandedSections.standard && (
        <div className="px-3 pb-3 space-y-3">
            ...
            return (
              <div key={key}>
                ...
                {type === "boolean" ? (
                  ...
                  <Select value={String(val || "")} onValueChange={(v) => update(key, v)}>
          ...
          <div>
            ...
            <TagInput
              value={Array.isArray(frontMatter.tags) ? frontMatter.tags as string[] : []}
          ...
          <div>
            ...
            <TagInput
              value={Array.isArray(frontMatter.categories) ? frontMatter.categories as string[] : []}
      ...
      {/* Custom Fields */}
      <SectionHeader label={`Custom Fields (${customKeys.length})`} section="custom" />
        ...
        <div className="px-3 pb-3 space-y-2">
          {customKeys.map((key) => (
            <div key={key} className="flex items-center gap-1.5">
              ...
              <Button variant="ghost" size="icon" className="h-6 w-6 flex-shrink-0 mt-4" onClick={() => remove(key)}>
                <X className="w-3 h-3" />
              </Button>
            </div>
          ))}
          ...
          <div className="space-y-1.5 pt-1">
            <div className="flex gap-1">
              ...
              />
              <Button
                variant="outline" size="icon" className="h-6 w-6 flex-shrink-0"
                onClick={() => {
