...
function parseMarkdown(md: string): string {
  let html = md
    ...
    .replace(
      /`([^`]+)`/g,
      '<code class="bg-muted/60 rounded px-1 py-0.5 text-sm font-mono">$1</code>'
    )
    // Headings
    .replace(
      /^#### (.+)$/gm,
    ...
    .replace(
      /^## (.+)$/gm,
      '<h2 class="text-xl font-bold mt-6 mb-3 font-display">$1</h2>'
    )
    .replace(
      /^# (.+)$/gm,
      '<h1 class="text-2xl font-bold mt-6 mb-3 font-display">$1</h1>'
    )
    // Blockquotes
    .replace(
      /^> (.+)$/gm,
      '<blockquote class="border-l-4 border-primary/40 pl-4 italic text-muted-foreground my-3">$1</blockquote>'
    )
    // Bold & Italic
  ...
  html = html.replace(
    /((<tr>[\s\S]*?<\/tr>\s*)+)/g,
    '<table class="w-full border-collapse my-4">$1</table>'
  );
...
export default function MarkdownPreview({ markdown, className }: Props) {
  ...
  return (
    <div
      className={cn(
        "prose-forge text-foreground text-sm leading-relaxed",
        className
      )}
