...
export function parseMarkdownFrontMatter(raw: string): {
  ...
  for (const line of yamlStr.split('\n')) {
    ...
    else if (!Number.isNaN(Number(value)) && value !== '') frontMatter[key] = Number(value);
...
export function serializeToMarkdown(frontMatter: FrontMatter, markdown: string): string {
  const lines = ['---'];

  for (const [key, value] of Object.entries(frontMatter)) {
    if (value === null || value === undefined) continue;
    if (Array.isArray(value)) lines.push(`${key}: [${value.map(item => `"${item}"`).join(', ')}]`);
