...
export default function SiteHealth() {
  ...
  const postsWithoutDesc =
    posts?.filter(
      p =>
        !p.frontMatter ||
        !(p.frontMatter as Record<string, unknown>).description
    ) || [];
  const assetsWithoutAlt =
    assets?.filter(a => !a.alt && a.mimeType?.startsWith("image/")) || [];
  ...
  const healthChecks = [
    ...
    },
    {
      ...
      detail: site?.isJekyll
        ? "_posts and _config.yml detected"
        : "Jekyll structure not confirmed",
      icon: FileText,
    },
    {
      label: "GitHub Pages",
    ...
    },
    {
      ...
      status: rateLimit
        ...
        : "info",
      detail: rateLimit
        ? `${rateLimit.rate?.remaining}/${rateLimit.rate?.limit} remaining`
        : "Unknown",
      icon: Cpu,
    },
    {
  ...
  const StatusIcon = ({ status }: { status: string }) => {
    if (status === "pass")
      return <CheckCircle2 className="w-4 h-4 text-forge-emerald" />;
    if (status === "warn")
      return <AlertCircle className="w-4 h-4 text-forge-amber" />;
    if (status === "fail")
      return <XCircle className="w-4 h-4 text-forge-rose" />;
              ...
              <Badge
                ...
                className={cn(
                  ...
                  status === "pass"
                    ...
                    : status === "warn"
                      ? "text-forge-amber border-forge-amber/30"
                      : status === "fail"
                        ? "text-forge-rose border-forge-rose/30"
            ...
            {[
              {
                ...
                color: "bg-forge-emerald",
              },
              {
                label: "Drafts",
                value: draftPosts.length,
                total: posts?.length || 1,
                color: "bg-forge-amber",
              },
              {
                label: "Scheduled",
                value: pendingScheduled.length,
                total: posts?.length || 1,
                color: "bg-forge-violet",
              },
          ...
          <CardContent className="space-y-2 text-sm">
            {postsWithoutDesc.length > 0 && (
              <div className="flex items-start gap-2 text-forge-amber">
                ...
                <span className="text-xs">
                  Add descriptions to {postsWithoutDesc.length} posts for better
                  SEO
                </span>
              </div>
            )}
            {assetsWithoutAlt.length > 0 && (
              <div className="flex items-start gap-2 text-forge-amber">
                <AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" />
                <span className="text-xs">
                  Add alt text to {assetsWithoutAlt.length} images for
                  accessibility
                </span>
              </div>
            )}
            {failedScheduled.length > 0 && (
              <div className="flex items-start gap-2 text-forge-rose">
                ...
                <span className="text-xs">
                  {failedScheduled.length} scheduled posts failed to publish
                </span>
              </div>
            )}
            {postsWithoutDesc.length === 0 &&
