...
import {
  FileText, Image, GitBranch, Globe, Plus, Clock, CheckCircle2,
  AlertCircle, XCircle, Activity, Cpu, Calendar, Zap, ArrowRight,
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (site && site.id !== activeSite?.id) setActiveSite(site as Parameters<typeof setActiveSite>[0]);
  }, [site]);
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <h1 className="font-display font-bold text-2xl">{site.owner}/{site.repo}</h1>
            {site.isJekyll && <Badge className="bg-forge-emerald/15 text-forge-emerald border-forge-emerald/30 text-xs">Jekyll</Badge>}
          </div>
          <div className="flex items-center gap-3 text-sm text-muted-foreground">
            ...
            {pagesStatus && (
              <div className="flex items-center gap-1">
                ...
                <a href={pagesStatus.html_url} target="_blank" rel="noopener noreferrer" className="hover:text-primary flex items-center gap-1">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { label: "Published Posts", value: publishedPosts.length, icon: CheckCircle2, color: "text-forge-emerald" },
          { label: "Drafts", value: draftPosts.length, icon: FileText, color: "text-forge-amber" },
          { label: "Assets", value: assets?.length || 0, icon: Image, color: "text-primary" },
          { label: "Scheduled", value: pendingScheduled.length, icon: Calendar, color: "text-forge-violet" },
        ...
        <div className="lg:col-span-2">
          ...
          <div className="space-y-2">
            {posts?.slice(0, 8).map((post) => (
              <Link key={post.id} href={`/editor/${siteId}/${encodeURIComponent(post.path)}`}>
                <div className="flex items-center gap-3 p-3 bg-card border border-border rounded-lg hover:border-primary/30 transition-colors cursor-pointer group">
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm truncate">{post.title || post.filename || post.path}</div>
                    <div className="flex items-center gap-2 mt-0.5">
                      <Badge variant="outline" className={cn("text-[10px] px-1.5 py-0 h-4",
                        post.status === "published" ? "text-forge-emerald border-forge-emerald/30" :
                        post.status === "scheduled" ? "text-forge-violet border-forge-violet/30" :
            ...
            <CardContent className="space-y-2 text-sm">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  <div className={cn("w-2 h-2 rounded-full", githubStatus?.connected ? "bg-forge-emerald" : "bg-forge-rose")} />
                  <span className="text-xs">{githubStatus?.connected ? "Connected" : "Disconnected"}</span>
                </div>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">Pages</span>
                <Badge variant="outline" className={cn("text-xs",
                  pagesStatus?.status === "built" ? "text-forge-emerald border-forge-emerald/30" : "text-muted-foreground"
                )}>
              ...
              {rateLimit && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-mono">{rateLimit.rate?.remaining}/{rateLimit.rate?.limit}</span>
                </div>
              )}
            </CardContent>
          </Card>
              ...
              <CardContent className="space-y-2">
                {pendingScheduled.slice(0, 3).map((s) => (
                  <div key={s.id} className="text-xs">
                    <div className="font-medium truncate">{s.draftPath.split("/").pop()}</div>
          ...
          <Card className="bg-card border-border">
            ...
            <CardContent className="space-y-1.5">
              {[
                ...
                { label: "Upload Asset", href: `/assets/${siteId}`, icon: Image },
                { label: "Manage Themes", href: `/themes/${siteId}`, icon: Zap },
                { label: "Site Health", href: `/health/${siteId}`, icon: Activity },
