...
export default function RepoPicker() {
  ...
  const handleSelectRepo = async (repo: {
    ...
    try {
      ...
      const site = freshSites?.find(
        (s: { owner: string; repo: string }) =>
          s.owner === repo.owner.login && s.repo === repo.name
      );
      if (site) setActiveSite(site as Parameters<typeof setActiveSite>[0]);

      toast.success(
        `${isJekyll ? "Jekyll site detected!" : "Repository connected."} Opening workspace...`
      );
  ...
  const handleSelectSaved = (
    site: typeof savedSites extends (infer T)[] | undefined ? T : never
  ) => {
    if (!site) return;
        ...
        <div className="max-w-5xl mx-auto px-6 py-5 flex items-center justify-between">
          ...
          {githubStatus?.connected && (
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <Badge
                variant="outline"
                className="text-forge-emerald border-forge-emerald/30 bg-forge-emerald/10 text-xs"
              >
                Connected
              </Badge>
            </div>
              ...
              <div className="flex flex-col items-center text-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-sm max-w-md">
                    Jekyll Forge uses a GitHub Personal Access Token to read and
                    write to your repositories. Your token is stored securely
                    and never logged.
                  </p>
                </div>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {site.selectedBranch || site.defaultBranch}
                      </span>
                      {site.isJekyll && (
                        <Badge
                          variant="outline"
                          className="text-[10px] px-1.5 py-0 h-4 text-forge-emerald border-forge-emerald/30"
                        >
                          Jekyll
                        </Badge>
                      )}
