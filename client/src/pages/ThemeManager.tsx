...
const POPULAR_THEMES = [
  {
    name: "minima",
    displayName: "Minima",
    desc: "Jekyll's default theme — clean, minimal, and versatile",
    stars: "3.2k",
  ...
  {
    name: "just-the-docs",
    displayName: "Just the Docs",
    desc: "A modern, high customizable, responsive Jekyll theme for documentation",
    stars: "6.8k",
  ...
  },
  {
    ...
    displayName: "Beautiful Jekyll",
    desc: "A ready-to-use template to help you create a beautiful website quickly",
    stars: "5.1k",
    url: "https://github.com/daattali/beautiful-jekyll",
  },
  {
    name: "al-folio",
    displayName: "Al-Folio",
    desc: "A beautiful, simple, clean, and responsive Jekyll theme for academics",
    stars: "10.2k",
...
const POPULAR_PLUGINS = [
  {
    name: "jekyll-seo-tag",
    desc: "Add metadata tags for search engines and social networks",
    category: "SEO",
  },
  {
    name: "jekyll-sitemap",
    desc: "Silently generate a sitemaps.org compliant sitemap",
    category: "SEO",
  },
  {
    name: "jekyll-feed",
    desc: "Generate an Atom feed of your Jekyll posts",
    category: "Content",
  },
  {
    name: "jekyll-paginate",
    desc: "Pagination generator for Jekyll",
    category: "Navigation",
  },
  {
    name: "jekyll-archives",
    desc: "Automatically generate post archives by dates, tags, and categories",
    category: "Navigation",
  },
  {
    name: "jekyll-redirect-from",
    desc: "Seamlessly specify multiple redirections URLs for your pages and posts",
    category: "Utility",
  },
  {
    name: "jekyll-compress-html",
    desc: "Compresses HTML in pure Liquid",
    category: "Performance",
  },
  {
    name: "jekyll-minifier",
    desc: "Minifies HTML, CSS, and JavaScript",
    category: "Performance",
  },
];
...
export default function ThemeManager() {
  ...
  const updateConfig = trpc.github.updateJekyllConfig.useMutation({
    onSuccess: (_, vars) => {
      toast.success(
        vars.theme
          ? `Theme updated to ${vars.theme}`
          : vars.addPlugin
            ? `Plugin ${vars.addPlugin} added`
            : `Plugin removed`
      );
  ...
  const filteredThemes = POPULAR_THEMES.filter(
    t =>
      !search ||
      t.name.includes(search.toLowerCase()) ||
      t.displayName.toLowerCase().includes(search.toLowerCase())
  );
  const filteredPlugins = POPULAR_PLUGINS.filter(
    p =>
      !search ||
    ...
    <div className="p-6 max-w-4xl mx-auto">
          ...
          {filteredThemes.map(theme => (
            <Card
              ...
              className={cn(
                ...
                currentTheme === theme.name && "border-primary/50 bg-primary/5"
              )}
            >
              <CardContent className="pt-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {currentTheme === theme.name && (
                        <Badge className="bg-forge-emerald/15 text-forge-emerald border-forge-emerald/30 text-[10px] px-1.5 py-0 h-4">
                          Active
                        </Badge>
                      )}
                    </div>
                ...
                <div className="flex gap-2">
                  <Button
                    variant={
                      currentTheme === theme.name ? "secondary" : "outline"
                    }
                    size="sm"
          ...
          {filteredPlugins.map(plugin => {
            ...
            return (
              <div
                key={plugin.name}
                className={cn(
                  "flex items-center gap-4 p-4 bg-card border border-border rounded-xl transition-all hover:border-primary/30",
                  isInstalled && "border-forge-emerald/30 bg-forge-emerald/5"
                )}
              >
      ...
      <Dialog open={showActionsDialog} onOpenChange={setShowActionsDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Apply {selectedItem?.type === "theme" ? "Theme" : "Plugin"}:{" "}
              {selectedItem?.name}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 text-sm">
            <p className="text-muted-foreground">
              To apply this {selectedItem?.type}, update your{" "}
              <code className="bg-muted px-1 rounded">_config.yml</code> and{" "}
              <code className="bg-muted px-1 rounded">Gemfile</code>:
            </p>
...
gem "${selectedItem?.name}"`}
            ...
            <p className="text-xs text-muted-foreground">
              Click <strong>Commit to GitHub</strong> to automatically update{" "}
              <code className="bg-muted px-1 rounded">_config.yml</code> in your
              repository. GitHub Pages will rebuild your site automatically
              after the commit.
            </p>
            <div className="flex gap-2">
              <Button
