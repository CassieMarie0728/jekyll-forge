...
const STATUS_CONFIG = {
  ...
  published: {
    color: "text-forge-emerald border-forge-emerald/30 bg-forge-emerald/10",
    icon: CheckCircle2,
    label: "Published",
  },
  failed: {
    color: "text-forge-rose border-forge-rose/30 bg-forge-rose/10",
    icon: XCircle,
    label: "Failed",
  },
  cancelled: {
...
export default function Scheduler() {
  ...
  const { data: heartbeatJobs } = trpc.scheduler.listHeartbeatJobs.useQuery({
    page: 1,
    pageSize: 50,
  });
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Automated post publishing for {site?.owner}/{site?.repo}
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          {
            label: "Published",
            value: scheduled?.filter(s => s.status === "published").length || 0,
            color: "text-forge-emerald",
          },
          {
            label: "Failed",
            value: scheduled?.filter(s => s.status === "failed").length || 0,
              ...
              {heartbeatJobs.jobs.slice(0, 5).map(job => (
                ...
                >
                  ...
                  <span className="flex items-center gap-1 ml-2 flex-shrink-0">
                    ...
                    {job.nextExecutionAt &&
                      ` · next: ${format(new Date(job.nextExecutionAt), "MMM d HH:mm")}`}
                  </span>
                </div>
              ))}
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="space-y-3">
                {pending.map(s => (
                  <div
                    key={s.id}
                    className="flex items-start gap-4 p-4 bg-card border border-border rounded-xl hover:border-forge-amber/30 transition-colors"
                  >
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground">
                          ...
                          {s.publishedAt &&
                            ` · Published ${formatDistanceToNow(new Date(s.publishedAt), { addSuffix: true })}`}
          ...
          {(!scheduled || scheduled.length === 0) && (
            <div className="text-center py-16 text-muted-foreground border border-dashed border-border rounded-xl">
              ...
              <p className="text-xs mt-1">
                Use the editor to schedule a post for future publishing
              </p>
            </div>
          )}
        </>
