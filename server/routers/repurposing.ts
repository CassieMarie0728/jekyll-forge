...
import { getRepurposingPrompt, getFormatMetadata, RepurposingFormat } from "../repurposingPrompts";
...
export const repurposingRouter = router({
  ...
  generate: protectedProcedure
    .input(z.object({
      postId: z.number(),
      siteId: z.number(),
      format: z.enum(["twitter", "linkedin", "tiktok", "youtube", "newsletter", "email", "podcast", "slides"]),
    }))
    .mutation(async ({ ctx, input }) => {
      ...
      if (!postContent.trim()) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "Post content is empty" });
      }

      try {
        // Build repurposing prompt
        const userPrompt = getRepurposingPrompt(input.format as RepurposingFormat, postContent);
        ...
        const response = await invokeLLM({
          messages: [
            {
              role: "system",
              content: aiSettings?.systemPrompt || "You are an expert content strategist specializing in repurposing blog content for different platforms. Create engaging, platform-appropriate content that maintains the core message of the original post.",
            },
        ...
        // Track usage
        if (response.usage) {
          await incrementAiUsage(ctx.user.id, response.usage.prompt_tokens || 0, response.usage.completion_tokens || 0);
        }

        // Get format-specific metadata
        const metadata = getFormatMetadata(input.format as RepurposingFormat, generatedContent);
        ...
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to generate repurposed content" });
      }
    }),
  ...
  update: protectedProcedure
    .input(z.object({
      ...
      status: z.enum(["generated", "approved", "published", "archived"]).optional(),
      isCustomized: z.boolean().optional(),
    }))
    .mutation(async ({ ctx, input }) => {
      const existing = await getRepurposedContentById(input.id, ctx.user.id);
      if (!existing) {
        throw new TRPCError({ code: "NOT_FOUND", message: "Repurposed content not found" });
      }
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const response = await invokeLLM({
        messages: [
          {
            role: "system",
            content: aiSettings?.systemPrompt || "You are an expert content strategist specializing in repurposing blog content for different platforms.",
          },
          { role: "user", content: userPrompt },
        ],
      });
      ...
      const generatedContent = response.choices[0]?.message?.content;
      if (!generatedContent || typeof generatedContent !== "string") {
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to regenerate content" });
      }
