...
| Mobile lint | **Pass with 30 warnings, 0 errors** | Local ESLint flat config now resolves correctly; safe screen-level route typing, unused-prop cleanup, and image-picker type cleanup reduced the baseline by 30 warnings during this remediation pass |
...
| P1 | **Mobile type/lint debt remains.** Lint completes but reports 30 warnings, chiefly legacy `any` boundaries and unused values. | Final mobile lint run | Continue removing legacy unused code and typing storage/navigation edges, then raise relevant warnings to errors. |
