...
| Mobile lint | **Pass with 22 warnings, 0 errors** | Local ESLint flat config now resolves correctly; safe screen-level route typing, unused-prop cleanup, image-picker typing, explicit haptics/notification contracts, draft-cache/storage typing, and narrowed tRPC/alert error handling reduced the baseline by 38 warnings during this remediation pass |
...
| P1 | **Mobile type/lint debt remains.** Lint completes but reports 22 warnings, chiefly legacy `any` boundaries and unused values. | Final mobile lint run | Continue removing legacy unused code and typing storage/navigation edges, then raise relevant warnings to errors. |
